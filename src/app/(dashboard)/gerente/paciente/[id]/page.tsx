...
interface PageProps {
    params: Promise<{ id: string }>; // This is actually the CPF as per our new routing structure
}
...
export default function GerentePatientDetailPage({ params }: PageProps) {
    ...
    const partnerCpf = searchParams.get('partnerCpf');
    ...
    const loadData = async () => {
        if (!cpf || !partnerCpf) return;
        const foundP = await fetchPatient(cpf, partnerCpf);
        setPatient(foundP);
        if (foundP) {
            // we should ideally fetch just the specific dentist, but fetchDentists caches or we can add fetchDentist to API
